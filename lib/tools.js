...
function projectModel(model, pin, meta) {
  ...
  return {
    ...
    pinMode: pin?.pinMode === 'preferred' ? 'preferred' : 'strict',
...
export function registerTools(ctx, { control, engine, store }) {
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        return text([
          ...
          `route registered=${value.routeRegistered} settings=${value.settingsAvailable ? 'available' : 'UNAVAILABLE (configuration cannot persist)'}`,
  ...
  ctx.tools.register(defineTool({
    name: 'cline_pass_pin',
    description: 'Persist one model\'s upstream pin. `upstreams` is the ordered candidate list, primary first (an empty array returns the model to automatic routing); `exclude` is never used; `pinMode` is "strict" or "preferred"; `sort` orders candidates by "cost", "ttft" or "tps" ("none" clears it). Only the fields you supply change.',
    parameters: {
      model: { type: 'string', required: true, description: 'Model id, e.g. "cline-pass/glm-5.2".' },
    ...
    async execute(args) {
      ...
      return {
        ...
        pinMode: stored.pinMode === 'preferred' ? 'preferred' : 'strict',
    ...
    async execute(args, exec) {
      ...
      if (action === 'test') {
        ...
        if (action === 'set') {
          ...
          // Merging can add or update, never drop: anything the previous pool
          // had and the new one does not must be unset explicitly, or a
          // shrinking `set` silently keeps the accounts it replaced.
          for (const stale of Object.keys(readConfig().accounts ?? {})) {
            if (next[stale] === undefined) await control.removeAccount(stale)
          }
        } else if (action === 'add') {
          ...
          const last = Object.keys(accounts).length === 1
          await control.removeAccount(key)
          note = last
        ...
        if (args.mode !== undefined) await control.updateConfig({ accountMode: args.mode === 'roundrobin' ? 'roundrobin' : 'single' })
        if (args.active !== undefined) await control.updateConfig({ activeAccount: nameOf(args.active) })
      }
      const config = readConfig()
      return {
        ok: true,
      ...
      render: (_args, value) => {
        ...
        return text([
          ...
          ...value.entries.map((entry) => {
            ...
            return `${when} ${entry.model} -> ${entry.provider || '(auto)'}${entry.canonical === '' ? '' : ` [${entry.canonical}]`} ${entry.ms}ms${entry.stream ? ' stream' : ''} account=${entry.account || '-'}${attempts}${error}`
          }),
        ].join('\n'))
      },
    },
