...
export { ClinePassAdapter, DEFAULT_CONTEXT_WINDOW, DEFAULT_MAX_TOKENS, DEFAULT_REQUEST_IMAGE_POLICY, DEFAULT_STREAM_IDLE_TIMEOUT_MS }
export { collectImageRefs, prepareRequestImages } from './adapter.js'
...
export { buildAttempts, injectPrefs, parseRouting } from './protocol.js'
...
export const Config = z.object({
  ...
  accountMode: z.string().default('single'),
  activeAccount: z.string().default(''),
  knownModels: z.array(z.string()).default(DEFAULT_MODELS),
  models: z.dict(ModelProfile).default({}),
  perModel: z.dict(PinProfile).default({}),
...
export function apply(ctx, config) {
  ...
  const resolveAccount = async () => {
    ...
    let picked
    if (cfg.accountMode === 'roundrobin' && enabled.length > 1) {
      picked = enabled[roundRobin % enabled.length]
      roundRobin = (roundRobin + 1) % 1000000000
    } else {
      picked = enabled.find(([key]) => key === cfg.activeAccount) ?? enabled[0]
    }
  ...
  engine.setPinReader((model) => live().perModel?.[model] ?? {})

  const adapter = new ClinePassAdapter({
    connection,
    modelMeta: (model) => store.metaOf(model),
    pin: (model) => live().perModel?.[model] ?? {},
    resolveAccount,
    discoveredContext,
    ...
    record: (model, info) => store.record({ model, ...info }),
   ...
   *
   * Nested dictionaries (`accounts`, `perModel`) are always written whole by
   * the caller, so the section's merge semantics never half-apply a change.
  ...
  const updateConfig = async (patch) => {
    ...
    await settings.update(DEFAULT_SETTINGS_NS, patch)
  }

  /**
   * Remove one key from a dictionary in this plugin's settings section.
   *
   * `settings.update` MERGES plain objects recursively, so a patch can add or
   * change a nested key but can never remove one: the key the caller dropped is
   * still in the layer underneath, survives the write, and comes straight back
   * on the next read. That is why a merge-only removal looks like a button that
   * does nothing. `settings.mutate` addresses one path and is the only write
   * mode that expresses a deletion, so every removal goes through here.
   *
   * @param path - `['accounts', name]`-style path inside the section.
   */
  const unsetConfig = async (path) => {
    const settings = ctx.get('settings')
    if (settings === undefined) {
      // No settings service: the in-memory config is the whole document, so the
      // key really can be dropped here.
      const [head, ...rest] = path.map(String)
      const draft = { ...liveConfig }
      if (rest.length === 0) {
        delete draft[head]
      } else {
        const nested = { ...(draft[head] ?? {}) }
        delete nested[rest[rest.length - 1]]
        draft[head] = nested
      }
      liveConfig = draft
      return
    }
    // Older settings services have no `mutate`. Failing loudly beats writing a
    // patch that silently leaves the key in place.
    if (typeof settings.mutate !== 'function') {
      throw new Error('this dsh settings service cannot delete a configuration key (no `mutate`), so the change would not stick; update dsh or edit the settings document by hand')
    }
    await settings.mutate(DEFAULT_SETTINGS_NS, [{ op: 'unset', path: path.map(String) }])
  }
  ...
  const control = {
    ...
    /**
     * Drop one account from the pool, clearing the active pointer when it named
     * that account.
     *
     * Both configuration faces (the `cline_pass_accounts` tool and the browser
     * panel) go through this: a merge-only write cannot remove a key, so the
     * knowledge of how a deletion is expressed lives here once.
     *
     * @param name - the account key, as `accounts()` reports it.
     */
    removeAccount: async (name) => {
      const key = String(name)
      await unsetConfig(['accounts', key])
      if (String(live().activeAccount ?? '') === key) await updateConfig({ activeAccount: '' })
    },
    refreshCatalog,
  }
  ...
  ctx.logger?.info('[cline-pass] provider route "%s" registered with %d model(s)', provider, modelIds().length)
}
