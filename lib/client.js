...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const VERDICT = { ok: 'available', limited: 'rate-limited', bad: 'not-pinnable', auth: 'auth-failed', unknown: 'unknown' }
    ...
    const CSS = [
      ...
      '.cp-tag{border:0.5px solid var(--dsw-alias-border-l2);border-radius:4px;padding:0 5px;color:var(--dsw-alias-label-secondary);white-space:nowrap;flex:none}',
    ...
    class PanelController {
       ...
       *
       * The rows are returned rather than parked on the snapshot: `run` already
       * stores the action's value, so the caller renders what it awaited.
       */
     ...
     * The models card.
     *
     * The list is long enough (one row per subscription model plus its channel
     * detail) that it can be folded away, which lets the accounts above it stay
     * on screen; the count in the title still says how many the route serves
     * while it is folded.
     */
    function ModelsCard(props) {
      ...
      return h('div', { className: 'cp-card' }, [
        h('div', { key: 'title', className: 'cp-card-title' }, [
          ...
          h('span', { key: 'actions', className: 'cp-row' }, [
            h('button', {
              key: 'fold', type: 'button', className: 'cp-btn',
              'aria-expanded': open,
              onClick: () => setOpen(!open),
     ...
     *
     * `entries === null` means "not read yet", which is NOT the same as
     * "nothing was recorded": the fold is closed by default, so an unread
     * history has to look unread rather than empty — reporting the empty state
     * before the first read is what made a full history look permanently blank.
     */
    function HistoryCard(props) {
      ...
      return h('div', { className: 'cp-panel' }, [
        ...
        entries === null
          ? h('div', { key: 'unread', className: 'cp-muted' }, t('busy'))
          : entries.length === 0
            ...
            : h('div', { key: 'rows', className: 'cp-history' }, entries.map((entry, index) => h('div', { key: `${entry.ts}-${index}`, className: 'cp-history-row' }, [
              ...
              // Which account of the pool served this call, as the round-robin
              // picks them. A request that never reached a channel has none.
              h('span', { key: 'a', className: 'cp-tag', title: t('accounts') }, entry.account || '—'),
    ...
    function Panel(props) {
      ...
      // `null` = not read yet; an array (possibly empty) = read. The calls that
      // fill these rows record on the host, so the only way the list ever
      // appears is by asking for it. The total travels with the read, so the
      // summary and the rows can never disagree about how much ran.
      const [history, setHistory] = React.useState(null)
      const [historyTotal, setHistoryTotal] = React.useState(null)
      ...
      // The history rows are the one thing the snapshot does not carry, so they
      // only ever appear by asking: once the snapshot is here, read them, and
      // read them again whenever the fold is reopened. Keyed on a boolean, so a
      // publish that swaps `data` for an equal-shaped object cannot loop.
      const loaded = state.data !== null
      React.useEffect(() => {
        if (loaded) void actions.loadHistory(25)
      }, [loaded])

      const data = state.data
      const disabled = state.busy !== null || (data !== null && data.settingsAvailable === false)
      const ready = data?.ready === true
      // A read is fresher than the snapshot it arrived with.
      const recorded = historyTotal ?? Number(data?.historySize ?? 0)
      ...
      return h('div', { className: 'cp-panel' }, [
        ...
        // Diagnostic detail, so it stays folded away until asked for — and the
        // summary carries the recorded count, so a closed fold still says
        // whether anything ran at all.
        h('details', {
          key: 'history',
          className: 'cp-card',
          // Opening the fold is the read: every other field arrives with the
          // snapshot, so nothing else would ever ask for these rows.
          onToggle: (event) => {
            if (event.currentTarget.open === true) actions.loadHistory(25)
          },
        }, [
          h('summary', { key: 'sum', className: 'cp-card-title' }, [
            h('span', { key: 'l' }, t('history')),
            recorded > 0 ? h('span', { key: 'n', className: 'cp-muted' }, fill(t('historyCount'), { n: recorded })) : null,
          ]),
