...
/**
 * Lead-in text for the user message that carries images recovered from tool
 * results, matching the wording the shipped DeepSeek adapter uses.
 */
 ...
 * Matches @deepseek-ai/dsh-attachment requestImageDimensions geometry.
 */
...
/**
 * Compute the ImageRequestTarget for one image reference.
 *
 * Carries BOTH contract shapes on purpose:
 *   - `{ width, height, maxBytes }` — required by harness >= 0.1.6 (validateTarget)
 *   - `{ maxPixels, maxBytes }`    — required by harness < 0.1.6 (validatePolicy)
 * Each harness version reads the fields it validates and ignores the others, so
 * one target satisfies both and the adapter stays usable across the upgrade.
 *
 * @param ref - image attachment reference.
 * @param policy - route policy (maxPixels, maxBytes, or explicit width/height).
 * @returns { width: number, height: number, maxPixels: number, maxBytes: number }
 */
function resolveImageTarget(ref, policy) {
  const maxBytes = Number.isSafeInteger(policy?.maxBytes) && policy.maxBytes > 0
    ? policy.maxBytes
    : DEFAULT_REQUEST_IMAGE_POLICY.maxBytes
  const maxPixels = Number.isSafeInteger(policy?.maxPixels) && policy.maxPixels > 0
    ? policy.maxPixels
    : DEFAULT_REQUEST_IMAGE_POLICY.maxPixels

  if (Number.isSafeInteger(policy?.width) && policy.width > 0
    && Number.isSafeInteger(policy?.height) && policy.height > 0) {
    return { width: policy.width, height: policy.height, maxPixels, maxBytes }
  }

  const srcWidth = Number.isSafeInteger(ref?.width) && ref.width > 0 ? ref.width : 1024
  const srcHeight = Number.isSafeInteger(ref?.height) && ref.height > 0 ? ref.height : 1024

  const dims = requestImageDimensions(srcWidth, srcHeight, maxPixels)
  return {
...
export async function prepareRequestImages(messages, attachments, policy, signal) {
  ...
  const prepared = await Promise.all(ordered.map((ref) => {
    const target = resolveImageTarget(ref, policy)
...
/**
 * Resolve one image block into its OpenAI wire part.
 *
 * @throws {LlmError} UNSUPPORTED_CONTENT when its request bytes were not prepared.
 */
function imagePart(block, images) {
  ...
  return { type: 'image_url', image_url: { url: `data:${version.mediaType};base64,${toBase64(version.data)}` } }
}

/** Recursively collect image parts from blocks, including nested tool results. */
function collectImageParts(blocks, images, target = []) {
  for (const block of blocks ?? []) {
    if (block.type === 'image') target.push(imagePart(block, images))
    else if (block.type === 'tool-result') collectImageParts(block.content, images, target)
  }
  return target
}
 ...
 *
 * An image nested in a tool result (read_image, a screenshot tool) has no
 * OpenAI `tool` wire spelling, so it rides a following user message instead —
...
function serializeMessages(messages, images) {
  ...
  for (const message of messages) {
    ...
    for (const block of message.content ?? []) {
      ...
      else if (block.type === 'image') parts.push(imagePart(block, images))
    }
    const hasImage = parts.some((part) => part.type === 'image_url')
    const text = parts.filter((part) => part.type === 'text').map((part) => part.text).join('')
    if (hasImage || text.length > 0 || toolResults.length === 0) {
      flushToolImages()
      wire.push({ role: 'user', content: hasImage ? parts : text })
    }
    for (const result of toolResults) {
      ...
      collectImageParts(result.content, images, pendingToolImages)
    }
  }
  flushToolImages()
  return wire
