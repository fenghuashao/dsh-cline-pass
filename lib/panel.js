...
const VERDICT_RANK = { ok: 0, limited: 1, unknown: 2, bad: 3, auth: 4 }
...
export function createPanel({ control, engine, store }) {
  ...
  async function accountRemove(payload) {
    ...
    // A merge write would leave this account in the layer underneath and it
    // would reappear on the very next read, so the control surface owns the
    // delete.
    await control.removeAccount(name)
